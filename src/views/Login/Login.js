...
import Banner from '../../assets/images/UdeG.png';
    ...
    return (
        <div className = 'main-container'>
            ...
            <div className = 'banner-container'>
              <div>
                <Image src = {Banner} width = {350}/>
              </div>
              <h1>Servicio Social</h1>
            </div>
            <div className = 'login-container' style = {formType === 'login' ? {left: '30vw'} : {left: '30vw'}}>
              <h3 className = 'text-center'>{formType === 'login' ? 'Inicia Sesión' : 'Registrate'}</h3>
              <Form>
                {formType !== 'login' &&
                <Form.Row>
                  <Form.Group as = {Col} controlId="formBasicEmail">
                    <Form.Label className = "font-weight-bold">Nombre Completo</Form.Label>
                    <Form.Control className = 'input' type="text" placeholder="Ingresa tu nombre" onBlur = {(e) => setUserData({...userData, name: e.target.value})}/>
                  </Form.Group>
                </Form.Row>
                }
                <Form.Row>
                  <Form.Group as = {Col} controlId="formBasicEmail">
                    ...
                    <Form.Control className = 'input' type="email" placeholder="Ingresa tu email" onBlur = {(e) => setUserData({...userData, email: e.target.value})}/>
                  </Form.Group>
                </Form.Row>
                <Form.Row>
                  <Form.Group as = {Col} controlId="formBasicPassword">
                    <Form.Label className = 'font-weight-bold'>Contraseña</Form.Label>
                    <Form.Control className = 'input' type="password" placeholder="Ingresa tu contraseña" onBlur = {(e) => setUserData({...userData, password: e.target.value})}/>
                  </Form.Group>
                  {formType !== 'login' &&
                    <Form.Group as = {Col} controlId="formBasicPassword">
                      <Form.Label className = 'font-weight-bold'>Confirmar contraseña</Form.Label>
                      <Form.Control className = 'input' type="password" placeholder="Nuevamente..." onBlur = {(e) => setUserData({...userData, confirmPassword: e.target.value})}/>
                    </Form.Group>
                  }
                </Form.Row>
                <ButtonToolbar className = 'justify-content-around'>
                  ...
                  <Button variant="outline-dark" block onClick = {async () => {
                    if(formType === 'signin')
                    {
                      try
                      {
                        ...
                        {
                          Swal.fire('Contraseñas...','Las contraseñas no coinciden', 'question')
                        }
                      }catch(err)
                      {
                        Notification.error({
